...
public class SingleMediaActivity extends SharedMediaActivity implements BaseMediaFragment.MediaTapListener {
    ...
    private void loadAlbumsLazy(Intent intent) {
        ...
        Disposable disposable = QueryHelper.getMedia(getApplicationContext(), album)
    ...
    private void updatePageTitle(int position) {
        //getSupportActionBar().setTitle(getString("%d of %d",  position +1 , adapter.getCount()));
        String temp  = String.format("%d of %d", position +1, adapter.getCount());
